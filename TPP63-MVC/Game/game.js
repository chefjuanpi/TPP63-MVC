var G = { // G: Game
  ...
  FillGrid: function() {
    ...
    for (var x = 0; x < G.Width; x++) {
      ...
      for (var y = 0; y < G.Height; y++) {
        ...
        //console.log("X: " + G.Grid[x][y].X + " ;Y: " + G.Grid[x][y].Y);
      }
    }
  },
  ...
  UpdateScore: function(clusterSize) {
    G.Score += clusterSize*clusterSize * 10;
  },
  ...
  OnClick: function(e) {
    ...
    if (cluster.length >= G.CLUSTER_MIN_SIZE) {
      G.UpdateScore(cluster);
      G.ScoreUpdated(G.Score);
  ...
  ScoreUpdated: function(newScore) {}
};